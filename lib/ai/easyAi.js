...
export const computersChoice = (arr, ai, player) => {
  ...
  let safeOrNot = findsIfMoveIsSafe(arr, mini, playable, ai, player)
  let coOrds = [mini[0]]
  ...
  makesChoice(coOrds, win, deny, continue2, continueAndMultiNew,
    continueAndNew, continue1, startsNewLine, playable, safeOrNot)
  return {mini: coOrds[0], cell: coOrds[1]}
}

const makesChoice = (coOrds, win, deny, continue2, continueAndMultiNew,
  continueAndNew, continue1, startsNewLine, playable, safeOrNot) => {
  let notSafe = [] // do something with this
  let dead = []
  if (win.length) {
...
const findsIfMoveIsSafe = (arr, mini, willPlay, ai, player) => { // augment to consider not sending to a game where human can deny
  ...
  for (let i = 0; i < willPlay.length; i++) {
    if (arr[willPlay[i]][0].wonBy) {
      ...
      couldBeWon.length && mini[0] !== i ? safeOrNot.push({status: 'notSafe', mini: Number(willPlay[i])})
