  ...
  if (coOrds.length < 2) {
    for (let j = 0; j < couldWin.length; j++) {
      ...
      isOwned.includes(couldWin[j][1]) &&
        ...
        coOrds.push(Number(willWin[j])) // can push to coOrds //
      }
    }
    return coOrds
  }
  ...
  if (aiOwns.length >= 1 && coOrds.length < 2) {
    ...
    if (willPlay.length >= 1 && coOrds.length < 2) {
      ...
      if (total > 1 && coOrds.length < 2) {
        linesConverge(coOrds, willPlay) // can push to coOrds //
        ...
        movesThatContainCounted.length
          ? linesConverge(coOrds, movesThatContainCounted) // can push to coOrds //
          : coOrds.push(counted[randomNum(counted.length)]) // can push to coOrds //
      }
    }
  }
}
...
const lastDitchMove = (coOrds, playable) => {
  if (playable.includes('4') && coOrds.length < 2) {
    coOrds.push(4) // can push to coOrds //
  } else {
    let index = randomNum(playable.length)
    coOrds.push(Number(playable[index])) // can push to coOrds //
  }
  return coOrds
}
