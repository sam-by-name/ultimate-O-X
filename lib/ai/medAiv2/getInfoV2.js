...
export const findsWin = (aiOwns, playable, mini, aiDb) => {
  let win = []
  for (let j = 0; j < couldWin.length; j++) {
    if (aiOwns.includes(couldWin[j][0]) &&
        aiOwns.includes(couldWin[j][1]) &&
      playable.includes(willWin[j])) {
      win.push(Number(willWin[j]))
      aiDb[mini[0]][willWin[j]].win = true
    }
  }
  return win
}

export const findsDeny = (playerOwns, playable, mini, aiDb) => {
  let deny = []
  for (let j = 0; j < couldWin.length; j++) {
    if (playerOwns.includes(couldWin[j][0]) &&
        playerOwns.includes(couldWin[j][1]) &&
          playable.includes(willWin[j])) {
      deny.push(Number(willWin[j]))
      aiDb[mini[0]][willWin[j]].deny = true
    }
  }
  return deny
}
