...
export const findsPlayable = (arr, mini, ai, player, inner) => {
  let aiOwns = ''
  let playable = ''
  let playerOwns = ''
  let taken
  let alive
  for (let j = 0; j < 9; j++) {
    if (inner) { // I weep, please smooth edges
      taken = arr[mini[0]][j].takenBy
      alive = arr[mini[0]][j].isAlive
    } else {
      taken = arr[j][0].wonBy
      alive = !arr[j][0].wonBy
    }
