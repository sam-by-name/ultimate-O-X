...
const makesChoice = (arr, ai, player, mini, count, coOrds, win, deny, continue2, continueAndMultiNew,
  ...
  let dead = []
  if (win.length) {
    willWinWinOrDenyWin(arr, mini, ai, player, safeOrNot, true)
    isMoveSmart(win, coOrds, safeOrNot, notSafe, dead)
  }
  if (deny.length && coOrds.length < 2) {
    willWinWinOrDenyWin(arr, mini, ai, player, safeOrNot, false) // did this work?
...
const willWinWinOrDenyWin = (arr, mini, ai, player, safeOrNot, boo) => {
  let {aiOwns, playable, playerOwns} =
