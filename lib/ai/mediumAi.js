import {win, couldWin, willWin, createAiArr} from '../gameArrays'

export const computersChoice = (arr, ai, player) => {
  let aiArr = createAiArr()
  findsMiniGame(arr, aiArr)
  findsPlayable(arr, aiArr, ai, player)
  let safeDeadNotSafe = findsIfMoveIsSafe(arr, aiArr, ai, player)
  // let coOrds = [mini[0]]
...
const findsPlayable = (arr, aiArr, ai, player) => {
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      if (arr[i][j].takenBy === ai.name) {
        aiArr[i][j].ai = true
      } else if (arr[i][j].takenBy === player.name) {
        aiArr[i][j].human = true
      }
      if (arr[i][j].isAlive) {
        aiArr[i][j].playable = true
      }
    }
  }
  return aiArr
