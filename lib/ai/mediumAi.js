...
const makesChoice = (arr, ai, player, mini, count, coOrds, win, deny, continue2, continueAndMultiNew,
  ...
  let dead = []
  if (win.length) {
    willVicOrDenyVic(arr, mini, ai, player, coOrds, win, true)
    if (coOrds.length < 2) { isMoveSmart(win, coOrds, safeOrNot, notSafe, dead) }
  }
  if (deny.length && coOrds.length < 2) {
    willVicOrDenyVic(arr, mini, ai, player, coOrds, deny, false)
...
const willVicOrDenyVic = (arr, mini, ai, player, coOrds, winDeny, boo) => {
  let {aiOwns, playable, playerOwns} =
  ...
  let victory =
  findsWinOrDeny(whoOwns, playable)
  for (let i = 0; i < victory.length; i++) {
    if (victory[i] === mini[0]) { // if move in mini can win whole or prevent win, takes it
