  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      tempArr.push({
        ...
        isAlive: true,
        isPlayable: true,
        boundaryStyle: {border: '10px solid #0E0B16'}, // should it start as  lime?
        lastTaken: false,
...
export let win = ['012', '036', '048', '147', '246', '258', '345', '678']

export const winArr = () => {
  let jumbledWin = []
  for (let i = 0; i < 4; i++) {
    jumbledWin.push(win[0])
    win.shift()
    jumbledWin.unshift(win[win.length - 1])
    win.pop()
  }
  win = jumbledWin
  // console.log(jumbledWin)
  return jumbledWin
}
