...
const findsContinue1And2 = (counted, mini, aiDb) => {
...
const findsBigContinue1And2 = (counted, aiDb) => {
  for (let i = 0; i < counted.length; i++) {
    if (counted[i].count > 1) {
...
const splitsBigSorted = (sorted, aiDb) => {
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i].count > 1) {
      aiDb[sorted[i].value][i].continueBigAndMultiNew = true
    } else {
      aiDb[sorted[i].value][i].continueBigAndNew = true
    }
  }
}

const splitsSorted = (sorted, mini, aiDb) => {
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i].count > 1) {
...
export const findsLine2Continue = (aiOwns, playable, mini, aiDb, boo) => {
  if (aiOwns.length >= 1) {
    ...
    if (willPlay.length) {
      ...
      boo ? findsContinue1And2(counted, mini, aiDb)
        : findsBigContinue1And2(counted, aiDb)
      let continues = continuesAndStartsNew(playable, counted)
      let sorted = countNumberOfInstances(continues)
      boo ? splitsSorted(sorted, mini, aiDb)
        : splitsBigSorted(sorted, aiDb)
    }
  }
}
...
export const findsNewLine = (playable, mini, aiDb) => {
  let posLines = findsAllPlayableWins(playable)
  ...
  for (let i = 0; i < startsNew.length; i++) {
    if (i === temp && i < startsNew.length - 1) {
      if (startsNew[i].count > startsNew[i + 1].count) {
        aiDb[mini][startsNew[i].value].startsMostNew = true
      } else {
        temp++
        aiDb[mini][startsNew[i].value].startsMostNew = true
      }
    } else aiDb[mini][startsNew[i].value].startsNew = true
  }
}
