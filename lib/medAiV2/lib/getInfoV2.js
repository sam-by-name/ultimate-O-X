  ...
  for (let j = 0; j < couldWin.length; j++) {
    if (isOwned.includes(couldWin[j][0]) &&
      ...
      if (typeof boo === 'boolean') {
        ...
        findsVicOrVicDeny(arr, ai, human, coOrds, winOrDeny, tempAiDb, boo)
      }
    }
  }
  return winOrDeny
...
const findsVicOrVicDeny = (arr, ai, human, coOrds, winOrDeny, tempAiDb, boo) => {
  if (boo) {
    tempAiDb.win = true
    findsVictory(arr, ai, human, coOrds, winOrDeny, tempAiDb) // if finds vic, takes
  }
  if (typeof boo === 'boolean' && !boo) {
    tempAiDb.deny = true
    findsVictoryDeny(arr, ai, human, coOrds, winOrDeny, tempAiDb) // if finds denyVic
  }
}

// If a winning move is found it takes it
const findsVictory = (arr, ai, human, coOrds, win, tempAiDb) => {
  let {aiOwns, playable} =
  ...
  for (let i = 0; i < victory.length; i++) {
    if (victory[i] === coOrds[0]) {
      coOrds.push(win[0])
      tempAiDb.victory = true
    }
  }
}

export const findsVictoryDeny = (arr, ai, human, coOrds, tempAiDb) => {
...
const findsContinue1And2 = (counted, continue2, continue1, mini, aiDb) => {
  for (let i = 0; i < counted.length; i++) {
    if (counted[i].count > 1) {
      continue2.push(counted[i].value)
      aiDb[mini][counted[i].value].continue2 = true
    } else if (counted[i].count === 1) {
      continue1.push(counted[i].value)
      aiDb[mini][counted[i].value].continue1 = true
    }
  }
}
...
const splitsSorted = (sorted, continueAndMultiNew, continueAndNew, mini, aiDb) => {
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i].count > 1) {
      ...
      aiDb[mini][sorted[i].value].continueAndMultiNew = true
    } else {
      continueAndNew.push(sorted[i].value)
      aiDb[mini][sorted[i].value].continueAndNew = true
    }
  }
}
...
export const findsLine2Continue = (aiOwns, playable, mini, aiDb) => {
  ...
  if (aiOwns.length >= 1) {
    ...
    if (willPlay.length) {
      ...
      findsContinue1And2(counted, continue2, continue1, mini, aiDb)
...
export const findsNewLine = (coOrds, playable, mini, aiDb) => {
  ...
  for (let i = 0; i < startsNew.length; i++) {
    ...
    aiDb[mini][startsNew[i].value].startsNew = true
  }
