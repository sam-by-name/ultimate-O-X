  ...
  for (let j = 0; j < couldWin.length; j++) {
    if (isOwned.includes(couldWin[j][0]) &&
      ...
      if (typeof boo === 'boolean') {
        ...
        findsVicOrVicDeny(arr, ai, player, coOrds, winOrDeny, tempAiDb, boo)
      }
    }
  }
  return winOrDeny
...
const findsVicOrVicDeny = (arr, ai, player, coOrds, winOrDeny, tempAiDb, boo) => {
  if (boo) {
    tempAiDb.win = true
    findsVictory(arr, ai, player, coOrds, winOrDeny, tempAiDb) // if finds vic, takes
  }
  if (typeof boo === 'boolean' && !boo) {
    tempAiDb.deny = true
    findsVictoryDeny(arr, ai, player, coOrds, winOrDeny, tempAiDb) // if finds denyVic
  }
}

// If a winning move is found it takes it
const findsVictory = (arr, ai, player, coOrds, win, tempAiDb) => {
  let {aiOwns, playable} =
