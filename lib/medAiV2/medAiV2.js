...
const findsIfMoveIsSafe = (arr, mini, willPlay, ai, human, aiDb) => {
  ...
  for (let i = 0; i < willPlay.length; i++) {
    ...
    if (arr[willPlay[i]][0].wonBy) {
      ...
      couldBeWon.length // player can win and so destination is checked.  // removed because unsure if needed '&& mini[0] !== willPlay[i]'
        ? splitAndGradeNotSafe(arr, ai, human, aiDb, safeOrNot, mini[0], toPlay, couldBeWon[0], x)
...
const splitAndGradeNotSafe = (arr, ai, human, aiDb, safeOrNot, mini, toPlay, couldBeWon, x) => {
  ...
  } else if (freeWin(arr, ai, human, mini, toPlay, couldBeWon)) { // finds where human win would lead ai
    safeOrNot.push({status: 'freeWin', mini: toPlay}) && (x.freeWin = true)
  } else if (!freeWin(arr, ai, human, mini, toPlay, couldBeWon)) {
    safeOrNot.push({status: 'freeWin', mini: toPlay}) && (x.aiMoreWins = true) // not really more wins if initial move isn't a win
  // safeOrNot.push({status: 'notSafe', mini: toPlay}) && (x.notSafe = true)
}

const freeWin = (arr, ai, human, mini, toPlay, couldBeWon) => { // does human get freeWin
  let tempArr = JSON.parse(JSON.stringify(arr))
  mockOutArr(tempArr, mini, toPlay, ai) // ai 'pesudo taken 1st' move
  mockOutArr(tempArr, toPlay, couldBeWon, human)
  ...
  if (aiCouldWin.length) {
    mockOutArr(tempArr, couldBeWon, aiCouldWin[0], ai) // human 'pesudo taken 2nd' move
    return checksForHumanWin(tempArr, ai, human, aiCouldWin[0]) // does this work?
  } else return true
}

const checksForHumanWin = (tempArr, ai, human, aiCouldWin) => {
  let {playable, humanOwns} = // need to mock out arr with pseudo moves for this to work
  findsPlayable(tempArr, aiCouldWin, ai, human)
  let humanCouldWin = findsWinOrDeny(humanOwns, playable)
  if (humanCouldWin.length) {
    mockOutArr(tempArr, aiCouldWin, humanCouldWin[0], human)
    // ai's next turn ... or recursion
  } else return true
