...
const findsIfMoveIsSafe = (arr, mini, willPlay, ai, player, aiDb) => {
  ...
  for (let i = 0; i < willPlay.length; i++) {
    let target = Number[willPlay[i]]
    let x = aiDb[mini[0]][willPlay[i]]
    if (arr[willPlay[i]][0].wonBy) {
      safeOrNot.push({status: 'dead', mini: target})
      x.dead = true
    } else {
      let couldBeWon = findsCouldBeWon(arr, willPlay[i], ai.name, player.name)
      couldBeWon.length && mini[0] !== willPlay[i]
        ? safeOrNot.push({status: 'notSafe', mini: target}) && (x.notSafe = true)
        : safeOrNot.push({status: 'safe', mini: target}) && (x.safe = true)
    }
  }
  return safeOrNot
}

const findsCouldBeWon = (arr, mini, ai, player) => {
