...
const scans4Possibilities = (arr, ai, human, mini, aiDb, boo) => {
  ...
  for (let i = 0; i < mini.length; i++) {
    ...
    findsNewLine(playable, mini[i], aiDb)
...
const aiFirstGoScore = (x) => {
  if (x.victory) x.totScore += 100000
  ...
  if (x.harikari) x.totScore -= 100000
}

const ai2ndGoScore = (x, z) => {
  if (z.victory) x.totScore += 5000
...
const aiTurn = (arr, ai, human, x) => {
  ...
  for (let i = 0; i < aiPlayable.length; i++) {
    ...
    if (!z.futureHarikari) human2ndTurn(tempArr, ai, human, x)
  }
}
