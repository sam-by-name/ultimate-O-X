...
const findsIfMoveIsSafe = (arr, mini, willPlay, ai, human, aiDb) => { // willPlay === playable within current mini
  ...
  for (let i = 0; i < willPlay.length; i++) {
    ...
    if (arr[willPlay[i]][0].wonBy) {
      ...
      safeOrNot.push({status: 'leads2dead', mini: toPlay}) && (x.leads2dead = true)
    } else {
...
const gradeHumanWin = (tempArr, x, toPlay, couldBeWon, aiCouldDeny, aiCouldWin, safeOrNot) => {
  ...
  if (!x.win && !x.deny && tempArr[couldBeWon][0].wonBy) {
    status = 'safe'; x.safe = true //          random, 1 human win, 1 ai free turn
  } else if (x.win && !x.deny && tempArr[couldBeWon][0].wonBy) {
    status = 'safe'; x.safe = true //        1 ai win, 1 human win, 1 ai free turn
  } else if (!x.win && x.deny && tempArr[couldBeWon][0].wonBy) {
    status = 'safe'; x.safe = true //         ai deny, 1 human win, 1 ai free turn
  }
  else if (!x.win && !x.deny && aiCouldDeny.length) {
    status = 'poor'; x.poor = true //          random, 1 human win, 1 ai deny
  } else if (x.win && !x.deny && aiCouldDeny.length) {
    status = 'decent'; x.decent = true //    1 ai win, 1 human win, 1 ai deny
  } else if (!x.win && x.deny && aiCouldDeny.length) {
    status = 'notGood'; x.notGood = true // 1 ai deny, 1 human win, 1 ai deny
  }
  else if (!x.win && !x.deny && aiCouldWin.length) {
    status = 'trade'; x.trade = true //        random, 1 human win, 1 ai win
  } else if (x.win && !x.deny && aiCouldWin.length) {
    status = 'good'; x.good = true //        1 ai win, 1 human win, 1 ai win
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    status = 'decent'; x.decent = true //   1 ai deny, 1 human win, 1 ai win
  }
  else if (!x.win && !x.deny && !aiCouldWin.length) { // finds where human win would lead ai
    status = 'bad'; x.bad = true //            random, 1 human win, random
  } else if (x.win && !x.deny && !aiCouldWin.length) {
    status = 'trade'; x.trade = true //      1 ai win, 1 human win, random
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    status = 'poor'; x.poor = true //       1 ai deny, 1 human win, random
  } 
...
const gradeHumanDeny = (tempArr, x, toPlay, couldBeDenied, aiCouldDeny, aiCouldWin, safeOrNot) => {
  ...
  if (!x.win && !x.deny && tempArr[couldBeDenied][0].wonBy) {
    status = 'safe'; x.safe = true //        random, 1 human deny, 1 ai free turn
  } else if (x.win && !x.deny && tempArr[couldBeDenied][0].wonBy) {
    status = 'safe'; x.safe = true //      1 ai win, 1 human deny, 1 ai free turn
  } else if (!x.win && x.deny && tempArr[couldBeDenied][0].wonBy) {
    status = 'safe'; x.safe = true //       ai deny, 1 human deny, 1 ai free turn
  }
  else if (!x.win && !x.deny && aiCouldDeny.length) {
    status = 'poor'; x.poor = true //        random, 1 human deny, 1 ai deny
  } else if (x.win && !x.deny && aiCouldDeny.length) {
    status = 'good'; x.good = true //      1 ai win, 1 human deny, 1 ai deny
  } else if (!x.win && x.deny && aiCouldDeny.length) {
    status = 'decent'; x.decent = true // 1 ai deny, 1 human deny, 1 ai deny
  }
  else if (!x.win && !x.deny && aiCouldWin.length) {
    status = 'decent'; x.decent = true //    random, 1 human deny, 1 ai win
  } else if (x.win && !x.deny && aiCouldWin.length) {
    status = 'great'; x.great = true //    1 ai win, 1 human deny, 1 ai win
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    status = 'good'; x.good = true //     1 ai deny, 1 human deny, 1 ai win
  }
  else if (!x.win && !x.deny && !aiCouldWin.length) { // finds where human win would lead ai
    status = 'poor'; x.poor = true //        random, 1 human deny, random
  } else if (x.win && !x.deny && !aiCouldWin.length) {
    status = 'decent'; x.decent = true //  1 ai win, 1 human deny, random
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    status = 'trade'; x.trade = true //   1 ai deny, 1 human deny, random
  } 
  if (typeof status === 'string') {
    safeOrNot.push({status: status, mini: toPlay})
  }
