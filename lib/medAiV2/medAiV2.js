  ...
  for (let i = 0; i < playable.length; i++) {
    ...
    if (!x.harikari) {
      ...
      for (let j = 0; j < move.length; j++) {
        splitHumanMove(tempArr, ai, human, aiDb, mini, toPlay, move[j], x, boo)
      }
    }
  }
}
...
const splitHumanMove = (arr, ai, human, aiDb, mini, toPlay, humanMove, x, boo) => {
  ...
  let humanDeny = aiWinOrHumanDeny(arr, ai, human, mini) // used only in grade2DeadOr2Same
  let humanWin = humanWinOrAiDeny(arr, ai, human, mini) //  used only in grade2DeadOr2Same
  ...
  if (x.sends2Same || x.sends2Dead) grade2DeadOr2Same(x, humanVics, allHumanWins, allHumanDenies, humanDeny, humanWin)
