...
const gradesHumanVictory = (arr, ai, human, aiDb, humanVic) => {
  let humanWin = 0
...
const gradesMoves = (arr, mini, playable, ai, human, aiDb) => {
  ...
  for (let i = 0; i < playable.length; i++) {
    ...
    let humanWin = gradesHumanVictory(arr, ai, human, aiDb, humanVic) // spoiler, it's bad
    ...
    if (!couldBeWon.length && !couldBeDenied.length) {
      ...
      if (!x.harikari) {
        let {playable} = findsPlayable(arr, toPlay, ai, human)
        for (let t = 0; t < couldBeWon.length; t++) {
          splitHumanMove(arr, ai, human, aiDb, mini, toPlay, playable[t], x, humanVic, humanWin)
        }
      }
    }
    if (couldBeWon.length || couldBeDenied.length) { // player could win and so destination is checked for outcome.
      ...
      if (!x.harikari && couldBeWon.length) {
        ...
        for (let g = 0; g < couldBeDenied.length; g++) {
          ...
          splitHumanMove(arr, ai, human, aiDb, mini, toPlay, couldBeDenied[g], x, humanVic, humanWin, false)
        }
      } else x.safe = true
    } else x.safe = true
  }
...
const splitHumanMove = (arr, ai, human, aiDb, mini, toPlay, humanMove, x, humanVic, humanWin, boo) => {
  ...
  mockOutArr(tempArr, toPlay, humanMove, human) // human 'pseudo taken 2nd' move
  let aiCouldWin = aiWinOrHumanDeny(tempArr, ai, human, humanMove)
  let aiCouldDeny = humanWinOrAiDeny(tempArr, ai, human, humanMove)
  if (x.sends2Same && x.win) gradeSends2Same(x, humanVic, humanWin, humanDeny)
  else if (doesHumanSendToVic(aiDb, arr, ai, human, humanMove, aiCouldWin, x)) return true// would lead to ai vic needs GRADING instead of just return!
  if (!x.harikari && typeof boo !== 'boolean') gradeHumanRandom(tempArr, x, humanMove, aiCouldDeny, aiCouldWin)
  else if (!x.harikari && boo) gradeHumanWin(tempArr, x, humanMove, aiCouldDeny, aiCouldWin)
  else if (!x.harikari && typeof boo === 'boolean' && !boo) {
    gradeHumanDeny(tempArr, x, humanMove, aiCouldDeny, aiCouldWin)
  }
}
...
const gradeHumanRandom = (tempArr, x, isRandom, aiCouldDeny, aiCouldWin) => {
  ...
  if (!x.win && !x.deny && tempArr[isRandom][0].wonBy) {
    status = 'decent' //         random, 1 human random, 1 ai free turn // write function to check if free move can lead too safely won, mark safe trade.
  } else if (x.win && !x.deny && tempArr[isRandom][0].wonBy) {
    status = 'great' //        1 ai win, 1 human random, 1 ai free turn
  } else if (!x.win && x.deny && tempArr[isRandom][0].wonBy) {
    status = 'good' //          ai deny, 1 human random, 1 ai free turn
  }
  else if (!x.win && !x.deny && aiCouldWin.length) {
    status = 'decent' //         random, 1 human random, 1 ai win
  } else if (x.win && !x.deny && aiCouldWin.length) {
    status = 'great' //        1 ai win, 1 human random, 1 ai win
  } else if (!x.win && x.deny && aiCouldWin.length) {
    status = 'good' //        1 ai deny, 1 human random, 1 ai win
  }
  else if (!x.win && !x.deny && aiCouldDeny.length) {
    status = 'safe' //           random, 1 human random, 1 ai deny
  } else if (x.win && !x.deny && aiCouldDeny.length) {
    status = 'good' //         1 ai win, 1 human random, 1 ai deny
  } else if (!x.win && x.deny && aiCouldDeny.length) {
    status = 'decent' //      1 ai deny, 1 human random, 1 ai deny
  }
  else if (!x.win && !x.deny && !aiCouldWin.length) { // finds where human win would lead ai
    status = 'safe' //           random, 1 human random, random
  } else if (x.win && !x.deny && !aiCouldWin.length) {
    status = 'good' //         1 ai win, 1 human random, random
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    status = 'decent' //      1 ai deny, 1 human random, random
  } 
  if (typeof status === 'string') x[status] = true
}
