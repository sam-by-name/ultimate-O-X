...
const splitAndGradeNotSafe = (arr, ai, human, aiDb, safeOrNot, mini, toPlay, couldBeWon, x) => {
  ...
  } else if (!x.win && !x.Deny && !aiWinOrHumanDeny(tempArr, ai, human, couldBeWon)) { // finds where human win would lead ai
    safeOrNot.push({status: 'freeWin', mini: toPlay}) && (x.freeWin = true) //       random, 1 human win, no win or deny for ai
  } else if (!x.win && !x.Deny && aiWinOrHumanDeny(tempArr, ai, human, couldBeWon)) {
    safeOrNot.push({status: 'draw', mini: toPlay}) && (x.draw = true) //             random, 1 human win, 1 ai win
  } else if (!x.win && !x.Deny && humanWinOrAiDeny(tempArr, ai, human, couldBeWon)) {
...
const aiWinOrHumanDeny = (tempArr, ai, human, couldBeWon) => { // does human get freeWin
  let {playable, aiOwns} =
  findsPlayable(tempArr, couldBeWon, ai, human)
...
const humanWinOrAiDeny = (tempArr, ai, human, couldBeWon) => {
  let {playable, humanOwns} =
  findsPlayable(tempArr, couldBeWon, ai, human)
  let humanCouldWin = findsWinOrDeny(humanOwns, playable)
  if (humanCouldWin.length) {
    return true
  } else return false
}
