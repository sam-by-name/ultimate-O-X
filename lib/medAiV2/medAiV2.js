    ...
    if (!couldBeWon.length && !couldBeDenied.length) {
      ...
      if (!x.harikari) {
        ...
        for (let t = 0; t < couldBeWon.length; t++) {
          splitHumanMove(arr, ai, human, aiDb, mini, toPlay, playable[t], x)
        }
      }
    }
    if (couldBeWon.length || couldBeDenied.length) { // player could win and so destination is checked for outcome.
      ...
      if (!x.harikari && couldBeWon.length) {
        ...
        // work on loops so each move can have more than 1 grade, if human could have more than one win
          splitHumanMove(arr, ai, human, aiDb, mini, toPlay, couldBeWon[j], x, true)
        }
        ...
        for (let g = 0; g < couldBeDenied.length; g++) {
          ...
          splitHumanMove(arr, ai, human, aiDb, mini, toPlay, couldBeDenied[g], x, false)
        }
      } else x.safe = true
    } else x.safe = true
  }
...
const isMoveHarikari = (toPlay, x, humanVic, mini) => {
  for (let i = 0; i < humanVic.length; i++) {
...
const splitHumanMove = (arr, ai, human, aiDb, mini, toPlay, humanMove, x, boo) => {
  ...
  let humanVictories = findAndMarkHumanVictory(tempArr, ai, human, aiDb, humanVic) // spoiler, it's bad
  let humanDeny = aiWinOrHumanDeny(tempArr, ai, human, mini)
  let humanWin = humanWinOrAiDeny(tempArr, ai, human, mini)
  let allHumanWins = findsAllHumanWins(tempArr, ai, human)
...
const gradeGivesFreeGo = (x, humanVictories, allHumanWins, humanDeny, humanWin) => { // improve to include what ai could do? humanWin === count of moves that == humanVic
  ...
  else if (!x.win && x.deny && x.sends2Same && humanWin.length > 1) status = 'poor' //                             2SameDeny , win
  ...
  else if (!x.win && !x.deny && x.sends2Same && humanWin.length > 1) status = 'bad' //                           2SameRandom , win
...
const gradeHumanDeny = (tempArr, x, couldBeDenied, aiCouldDeny, aiCouldWin, humanVic, humanWin) => {
  ...
  if (!x.win && !x.deny && tempArr[couldBeDenied][0].wonBy) {
    ...
    status = 'great' //        ai win, human deny, ai win
  } else if (!x.win && x.deny && aiCouldWin.length) {
    status = 'good' //        ai deny, human deny, ai win
  }
  else if (!x.win && !x.deny && !aiCouldWin.length) { // finds where human win would lead ai
    status = 'poor' //        random, human deny, random
  } else if (x.win && !x.deny && !aiCouldWin.length) {
    status = 'decent' //      ai win, human deny, random
  } else if (!x.win && x.deny && !aiCouldWin.length) {
