        ...
        if (!x.harikari === !sendToSame) {
          for (let j = 0; j < couldBeWon.length; j++) {
            if (length === safeOrNot.length) {
              splitAndGradeNotSafe(arr, ai, human, aiDb, safeOrNot, mini, toPlay, couldBeWon[j], x)
            }
          }
        } // else if (x.harikari)
         // else if (couldBeDenied.length) // continue here just like above for human deny
...
const splitAndGradeNotSafe = (arr, ai, human, aiDb, safeOrNot, mini, toPlay, couldBeWon, x) => {
  ...
  doesNotSendToVic(aiDb, arr, ai, human, couldBeWon[0], aiCouldWin)
...
const doesNotSendToVic = (aiDb, arr, ai, human, couldBeWon, aiCouldWin) => {
