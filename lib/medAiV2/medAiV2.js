...
const gradesMoves = (arr, mini, playable, ai, human, aiDb) => {
  for (let i = 0; i < playable.length; i++) {
    ...
    let z = findsPlayable(arr, toPlay, ai, human)
    let {move, boo, h} = setsHumanMove(x, couldBeWon, couldBeDenied, z.playable)
    if (!x.harikari && (x.sends2Same || x.sends2Dead)) {
      let {allHuW, allHuD} = findsAllHumanWinsNDenies(arr, ai, human)
      let huVics = findsHumanVictory(arr, ai, human, aiDb)
...
const splitHumanMove = (arr, ai, human, aiDb, toPlay, humanMove, h, x, boo) => {
  ...
  let {aiMove, a2} = setsAiMove(x, aiCouldWin, aiCouldDeny, z.playable) //
  for (let i = 0; i < aiMove.length; i++) {
    let {tempAiDb, a} = doesFutureAiSendToVic(aiDb, arr, ai, human, humanMove, aiMove[i])
    doesFutureMoveSend2Dead(arr, tempAiDb, aiMove[i], humanMove, a)
    aiCouldWin.length ? mockOutArr(arr, humanMove, aiMove[i], ai, true)
      : mockOutArr(arr, humanMove, aiMove[i], ai, false)

    aiDb[humanMove][aiMove[i]].humanVic = false
    let {couldBeDenied, couldBeWon} = setsHumanWinsAndDenies(arr, ai, human, aiMove[i])
