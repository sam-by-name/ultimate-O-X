  ...
  for (let i = 0; i < mini.length; i++) {
    ...
    if (i + 1 === mini.length) {
      makesChoice(arr, ai, player, coOrds, win, deny, continue2, continueAndMultiNew,
        continueAndNew, continue1, startsNewLine, playable, safeOrNot, aiDb)
    }
  }
  return coOrds
}
...
const makesChoice = (arr, ai, player, coOrds, win, deny, continue2, continueAndMultiNew,
  ...
  if (deny.length && coOrds.length < 2) {
    // isVictoryDenySafe(arr, ai, player, aiDb, coOrds) // CONTINUE HERE!!! //
...
// const isVictoryDenySafe = (arr, ai, player, aiDb, coOrds) => {
