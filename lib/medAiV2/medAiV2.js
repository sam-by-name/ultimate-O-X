...
const findsHighestScore = (moves, coOrds) => {
  moves.sort((a, b) => a.totScore < b.totScore)
  moves[0].totScore === moves[1].totScore ? coOrds = randomize(moves, coOrds)
    : coOrds = moves[0].coOrds
  return coOrds
}

const randomize = (moves, coOrds) => {
  ...
  coOrds = sameScore[randomNum(sameScore.length)].coOrds
  return coOrds
}
