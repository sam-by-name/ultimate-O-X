...
const aiFirstGoScore = (x) => {
  if (x.victory) x.ai1stScore += 100000
  if (x.victoryDeny) x.ai1stScore += 200
  if (x.win) x.ai1stScore += 100
  if (x.deny) x.ai1stScore += 25
  if (x.continue2) x.ai1stScore += 25
  if (x.continue1) x.ai1stScore += 15
  if (x.continueAndMultiNew) x.ai1stScore += 30
  if (x.continueAnNew) x.ai1stScore += 20
  if (x.startsMostNew) x.ai1stScore += 20
  if (x.startsNew) x.ai1stScore += 10
  if (x.zeroEffect) x.ai1stScore -= 50
  if (x.sends2Dead) x.ai1stScore -= 50
  if (x.harikari) x.ai1stScore -= 1000000
}

const ai2ndGoScore = (z, x) => {
  let temp = 0
  if (z.victory) temp += 10000
  if (z.victoryDeny) temp += 100
  if (z.win) temp += 100
  if (z.deny) temp += 25
  if (z.continue2) temp += 25
  if (z.continue1) temp += 15
  if (z.continueAndMultiNew) temp += 30
  if (z.continueAnNew) temp += 20
  if (z.startsMostNew) temp += 20
  if (z.startsNew) temp += 10
  if (z.zeroEffect) temp -= 50
  if (z.sends2Dead) temp -= 50 // is leads2Dead
  if (z.harikari) temp -= 10000
  x.push(temp)
}

const humanFirstGoScore = (y, x) => {
  let temp = 0
  if (y.victory) temp -= 100000
  if (y.victoryDeny) temp -= 100
  if (y.win) temp -= 100
  if (y.deny) temp -= 25
  if (y.continue2) temp -= 25
  if (y.continue1) temp -= 15
  if (y.continueAndMultiNew) temp -= 30
  if (y.continueAnNew) temp -= 20
  if (y.startsMostNew) temp -= 20
  if (y.startsNew) temp -= 10
  if (y.zeroEffect) temp += 50
  if (y.sends2Dead) temp += 200 // this is more than the ai loses .. ??
  if (y.harikari) temp += 100000 // is ai futureVic
  x.push(temp)
}

const human2ndGoScore = (y, x) => {
  let temp = 0
  if (y.victory) temp -= 10000
  if (y.victoryDeny) temp -= 100
  if (y.win) temp -= 100
  if (y.deny) temp -= 25
  if (y.continue2) temp -= 25
  ...
  if (y.continueAndMultiNew) temp -= 30
  if (y.continueAnNew) temp -= 20
  if (y.startsMostNew) temp -= 15
  if (y.startsNew) temp -= 10
  if (y.zeroEffect) temp += 50
  if (y.sends2Dead) temp += 100 // this is more than the ai loses .. ??
  if (y.harikari) temp += 10000
  x.push(temp)
}
