...
const splitHumanWin = (arr, ai, human, aiDb, safeOrNot, mini, toPlay, couldBeWon, x) => {
  ...
  if (doesHumanSendToVic(aiDb, arr, ai, human, couldBeWon, aiCouldWin, safeOrNot, toPlay, x)) return // would lead to ai vic so move is decent and returns

  if (!x.win && !x.deny && tempArr[couldBeWon][0].wonBy) {
    safeOrNot.push({status: 'safeRandom', mini: toPlay}) && (x.safeRandom = true) //    random, 1 human win, 1 ai free turn
  } else if (x.win && !x.deny && tempArr[couldBeWon][0].wonBy) {
    safeOrNot.push({status: 'safeWin', mini: toPlay}) && (x.safeWin = true) //        1 ai win, 1 human win, 1 ai free turn
  } else if (!x.win && x.deny && tempArr[couldBeWon][0].wonBy) {
    safeOrNot.push({status: 'safeDeny', mini: toPlay}) && (x.safeDeny = true) //       ai deny, 1 human win, 1 ai free turn
  }
  else if (!x.win && !x.deny && aiCouldDeny.length) {
    safeOrNot.push({status: 'lose1Deny1', mini: toPlay}) && (x.lose1Deny1 = true) //    random, 1 human win, 1 ai deny
  } else if (x.win && !x.deny && aiCouldDeny.length) {
    safeOrNot.push({status: 'advAi', mini: toPlay}) && (x.advAi = true) //            1 ai win, 1 human win, 1 ai deny
  } else if (!x.win && x.deny && aiCouldDeny.length) {
    safeOrNot.push({status: 'twoDeny1Loss', mini: toPlay}) && (x.twoDeny1Loss = true) // 1 ai deny, 1 human win, 1 ai deny
  }
  else if (!x.win && !x.deny && aiCouldWin.length) {
    safeOrNot.push({status: 'drawRandom', mini: toPlay}) && (x.drawRandom = true) //    random, 1 human win, 1 ai win
  } else if (x.win && !x.deny && aiCouldWin.length) {
    safeOrNot.push({status: 'safe', mini: toPlay}) && (x.safe = true) //              1 ai win, 1 human win, 1 ai win
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    safeOrNot.push({status: 'advAi', mini: toPlay}) && (x.advAi = true) //           1 ai deny, 1 human win, 1 ai win
  }
  else if (!x.win && !x.deny && !aiCouldWin.length) { // finds where human win would lead ai
    safeOrNot.push({status: 'freeWin', mini: toPlay}) && (x.freeWin = true) //          random, 1 human win, random
  } else if (x.win && !x.deny && !aiCouldWin.length) {
    safeOrNot.push({status: 'drawRandom', mini: toPlay}) && (x.drawRandom = true) //  1 ai win, 1 human win, random
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    safeOrNot.push({status: 'lose1Deny1', mini: toPlay}) && (x.lose1Deny1 = true) // 1 ai deny, 1 human win, random
  } 
  else if (aiWinOrHumanDeny(tempArr, ai, human, toPlay, couldBeWon)) {
    safeOrNot.push({status: 'aiMoreWins', mini: toPlay}) && (x.aiMoreWins = true) // not really more wins if initial move isn't a win
...
const splitHumanDeny = (arr, ai, human, aiDb, safeOrNot, mini, toPlay, couldBeDenied, x) => {
  ...
  let tempAiDb = doesHumanSendToVic(aiDb, arr, ai, human, couldBeDenied, aiCouldWin)

  if (!x.win && !x.deny && tempArr[couldBeDenied][0].wonBy) {
    safeOrNot.push({status: 'safeRandom', mini: toPlay}) && (x.safeRandom = true) //    random, 1 human deny, 1 ai free turn
  } else if (x.win && !x.deny && tempArr[couldBeDenied][0].wonBy) {
    safeOrNot.push({status: 'safeWin', mini: toPlay}) && (x.safeWin = true) //        1 ai win, 1 human deny, 1 ai free turn
  } else if (!x.win && x.deny && tempArr[couldBeDenied][0].wonBy) {
    safeOrNot.push({status: 'safeDeny', mini: toPlay}) && (x.safeDeny = true) //       ai deny, 1 human deny, 1 ai free turn
  }
  else if (!x.win && !x.deny && aiCouldDeny.length) {
    safeOrNot.push({status: 'lose1Deny1', mini: toPlay}) && (x.lose1Deny1 = true) //    random, 1 human deny, 1 ai deny
  } else if (x.win && !x.deny && aiCouldDeny.length) {
    safeOrNot.push({status: 'advAi', mini: toPlay}) && (x.advAi = true) //            1 ai win, 1 human deny, 1 ai deny
  } else if (!x.win && x.deny && aiCouldDeny.length) {
    safeOrNot.push({status: 'twoDeny1Loss', mini: toPlay}) && (x.twoDeny1Loss = true) // 1 ai deny, 1 human deny, 1 ai deny
  }
  else if (!x.win && !x.deny && aiCouldWin.length) {
    safeOrNot.push({status: 'drawRandom', mini: toPlay}) && (x.drawRandom = true) //    random, 1 human deny, 1 ai win
  } else if (x.win && !x.deny && aiCouldWin.length) {
    safeOrNot.push({status: 'safe', mini: toPlay}) && (x.safe = true) //              1 ai win, 1 human deny, 1 ai win
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    safeOrNot.push({status: 'advAi', mini: toPlay}) && (x.advAi = true) //           1 ai deny, 1 human deny, 1 ai win
  }
  else if (!x.win && !x.deny && !aiCouldWin.length) { // finds where human win would lead ai
    safeOrNot.push({status: 'freeWin', mini: toPlay}) && (x.freeWin = true) //          random, 1 human deny, random
  } else if (x.win && !x.deny && !aiCouldWin.length) {
    safeOrNot.push({status: 'drawRandom', mini: toPlay}) && (x.drawRandom = true) //  1 ai win, 1 human deny, random
  } else if (!x.win && x.deny && !aiCouldWin.length) {
    safeOrNot.push({status: 'lose1Deny1', mini: toPlay}) && (x.lose1Deny1 = true) // 1 ai deny, 1 human deny, random
  } 
  else if (aiWinOrHumanDeny(tempArr, ai, human, toPlay, couldBeDenied)) {
    safeOrNot.push({status: 'aiMoreWins', mini: toPlay}) && (x.aiMoreWins = true) // not really more wins if initial move isn't a win
  }// safeOrNot.push({status: 'notSafe', mini: toPlay}) && (x.notSafe = true)
}
...
const doesHumanSendToVic = (aiDb, arr, ai, human, couldBeWon, aiCouldWin, safeOrNot, toPlay, x) => {
  if (aiCouldWin.length) {
    ...
    if (tempAiDb[couldBeWon][aiCouldWin[0]].victory) {
      safeOrNot.push({status: 'leads2Vic', mini: toPlay}) && (x.leads2Vic = true)
      return true
    }
  } else return false
}
