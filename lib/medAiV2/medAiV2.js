  ...
  for (let i = 0; i < willPlay.length; i++) {
    ...
    if (arr[willPlay[i]][0].wonBy) {
      ...
      couldBeWon.length // removed because unsure if needed '&& mini[0] !== willPlay[i]'
        ? splitAndGradeNotSafe(safeOrNot, couldBeWon, Number(willPlay[i]), x)
        : safeOrNot.push({status: 'safe', mini: Number(willPlay[i])}) && (x.safe = true)
    }
  }
  return safeOrNot
}

const splitAndGradeNotSafe = (safeOrNot, couldBeWon, willPlay, x) => {
  safeOrNot.push({status: 'notSafe', mini: willPlay}) && (x.notSafe = true)
}
