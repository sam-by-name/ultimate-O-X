...
const findsIfMoveIsSafe = (arr, mini, willPlay, ai, human, aiDb) => { // willPlay === playable within current mini
  ...
  for (let i = 0; i < willPlay.length; i++) {
    ...
    if (arr[willPlay[i]][0].wonBy) {
      ...
      if (couldBeWon.length) { // player would win and so destination is checked for outcome.  // removed because unsure if needed '&& mini[0] !== willPlay[i]'
        splitAndGradeNotSafe(arr, ai, human, aiDb, safeOrNot, mini,
          toPlay, couldBeWon[0], x)
...
const splitAndGradeNotSafe = (arr, ai, human, aiDb, safeOrNot, mini, toPlay, couldBeWon, x) => {
  ...
  if (aiDb[toPlay][couldBeWon].victoryDeny) { // move human could take would === victory
  ...
  else if (!x.win && !x.Deny && !aiWinOrHumanDeny(tempArr, ai, human, couldBeWon, aiDb)) { // finds where human win would lead ai
    safeOrNot.push({status: 'freeWin', mini: toPlay}) && (x.freeWin = true) //       random, 1 human win, random
  } else if (!x.win && !x.Deny && aiWinOrHumanDeny(tempArr, ai, human, couldBeWon, aiDb)) {
    safeOrNot.push({status: 'drawRandom', mini: toPlay}) && (x.drawRandom = true) // random, 1 human win, 1 ai win
  } else if (!x.win && !x.Deny && humanWinOrAiDeny(tempArr, ai, human, couldBeWon, aiDb)) {
    ...
    safeOrNot.push({status: 'safeDeny', mini: toPlay}) && (x.safeDeny = true) //         1 ai deny, 1 human win, 1 ai free turn
  }
...
const aiWinOrHumanDeny = (arr, ai, human, couldBeWon, aiDb) => { // does human get freeWin
  const {playable, aiOwns} =
  findsPlayable(arr, couldBeWon, ai, human)
  const aiCouldWin = findsWinOrDeny(aiOwns, playable)
  let tempAiDb = JSON.parse(JSON.stringify(aiDb))
  findsVictoryDeny(arr, ai, human, aiCouldWin, tempAiDb)
  const notDead = !arr[couldBeWon][aiCouldWin[0]].wonBy
  const notHumanVic = !tempAiDb[couldBeWon][aiCouldWin[0]].victoryDeny
  if (aiCouldWin.length && notDead && notHumanVic) {
    return true
  } else if (!aiCouldWin.length && notDead && notHumanVic) return false
}

const humanWinOrAiDeny = (arr, ai, human, couldBeWon, aiDb) => { // humanCouldWin == aiCouldDeny
  const {playable, humanOwns} =
  findsPlayable(arr, couldBeWon, ai, human)
  const humanCouldWin = findsWinOrDeny(humanOwns, playable)
  let x = JSON.parse(JSON.stringify(aiDb))
  findsVictoryDeny(arr, ai, human, humanCouldWin, x)
  const notDead = !arr[couldBeWon][humanCouldWin[0]].wonBy
  const notHumanVic = !x[couldBeWon][humanCouldWin[0]].victoryDeny
  if (humanCouldWin.length && notDead && notHumanVic) {
    return true
  } else if (!humanCouldWin.length && notDead && notHumanVic) return false
}
