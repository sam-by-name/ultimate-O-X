...
const gradesMoves = (arr, mini, playable, ai, human, aiDb) => {
  ...
  for (let i = 0; i < playable.length; i++) {
    ...
    if (!couldBeWon.length && !couldBeDenied.length) {
    ...
    } else if (couldBeWon.length || couldBeDenied.length) { // player could win and so destination is checked for outcome.
      isMoveHarikari(toPlay, x, humanVic)
      ...
      } else x.safe = true // all conditions need to be found and these 'safe' if nothing other need to go!
    }
  }
}
...
const splitHumanMove = (arr, ai, human, aiDb, mini, toPlay, humanMove, x, boo) => {
  ...
  if (x.sends2Same || x.sends2Dead) grade2DeadOr2Same(x, humanVictories, allHumanWins, humanDeny, humanWin) // if this happens, done check other grades
